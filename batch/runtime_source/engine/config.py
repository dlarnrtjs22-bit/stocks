...
class SignalConfig:
    ...
    # LLM 분석 결과가 없을 때 기본 뉴스 점수
    default_news_score: int = 1
    # 신선한 종목 뉴스가 없을 때는 가점을 주지 않음
    no_news_score: int = 0
    # 종가배팅 뉴스 유효 구간: 전일 장마감 이후 + 당일
    news_prev_day_cutoff_hour: int = 15
    news_prev_day_cutoff_minute: int = 30
    # 운영 기본값: synthetic 시세/수급 fallback 사용 안함
    allow_synthetic_market_data_fallback: bool = False
    # 운영 기본값: synthetic 뉴스 fallback 사용 안함
    allow_synthetic_news_fallback: bool = False
